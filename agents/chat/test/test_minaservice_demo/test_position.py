...
async def main():
    username = config.get_env("MI_USERNAME")
    password = config.get_env("MI_PASSWORD")
    ...
    async with ClientSession() as session:
        ...
        sleep(5)
        set_position_response = await mina_service.ubus_request(
            device_id,
            "player_set_positon",
            "mediaplayer",
            {"media":"app_ios","position":20000}
        )
